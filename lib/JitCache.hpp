...
public:
  uint64_t hash(llvm::StringRef ModuleUniqueId, llvm::StringRef FnName,
                const RuntimeConstant *RC, int NumRuntimeConstants) const {
    llvm::ArrayRef<RuntimeConstant> Data(RC, NumRuntimeConstants);
  ...
  void insert(uint64_t HashValue, Function_t FunctionPtr, llvm::StringRef FnName,
...
private:
  ...
  llvm::DenseMap<uint64_t, JitCacheEntry> CacheMap;
